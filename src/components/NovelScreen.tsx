...
const NovelScreen: React.FC<NovelScreenProps> = ({ onBack }) => {
  ...
  const [galleryBackground, setGalleryBackground] = useState<string>(() => {
    return localStorage.getItem('novel_gallery_background_v3') || '';
  });
  ...
  useEffect(() => {
    localStorage.setItem('novel_gallery_background_v3', galleryBackground);
  }, [galleryBackground]);
  ...
  const handleImageUrl = (type: 'cover' | 'editorBg' | 'npcBg' | 'galleryBackground') => {
    ...
    if (url && url.trim()) {
      ...
      if (type === 'cover') updateCurrentNovel({ coverImage: result });
      else if (type === 'editorBg') updateCurrentNovel({ editorBackgroundImage: result });
      else if (type === 'npcBg') updateCurrentNovel({ npcGlobalBackground: result });
      else if (type === 'galleryBackground') setGalleryBackground(result);
    }
  };
...
- PHẢI VIẾT ĐỦ ĐỘ DÀI YÊU CẦU (${nextChapterLength || currentNovel.chapterLength} chữ/từ), TUYỆT ĐỐI KHÔNG DỪNG LẠI CHO ĐẾN KHI ĐỦ DUNG LƯỢNG.`;
  ...
  const handleGenerateGossip = async () => {
    ...
    try {
      const systemInstruction = `Bạn là một nhóm ${currentNovel.npcCount || 500} NPC (độc giả ảo) đang theo dõi câu chuyện "${currentNovel.storyName}".
Nhiệm vụ: Hãy tạo ra một cuộc hội thoại sôi nổi, cãi nhau, khen ngợi, bình luận về nội dung chương truyện vừa đọc.
YÊU CẦU:
1. Tạo ra khoảng 40-60 bình luận từ các NPC khác nhau (mô phỏng hội nhóm ${currentNovel.npcCount || 500} người).
